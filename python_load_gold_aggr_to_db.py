...
from sqlalchemy import create_engine, types, text # Importar 'text' para comandos SQL brutos
...
log_file_path = 'load_gold_aggr_to_db.log' # Log específico para o carregamento agregado
logging.basicConfig(
    level=logging.INFO,
...
GOLD_AGGR_INPUT_PATH = os.path.join(BASE_DIR, 'gold', 'scr-agregado')
...
def carregar_gold_to_postgresql(df_gold: pd.DataFrame, db_engine, table_name: str, if_exists_mode: str):
    ...
    try:
        ...
        # PostgreSQL prefere nomes em minúsculas e sem caracteres especiais.
        # Nomes de colunas do Pandas podem ter maiúsculas e caracteres como '.'.
        # Isso converte para minúsculas e substitui caracteres indesejados por '_'.
        ...
        # Define um mapeamento explícito para tipos de Pandas para tipos de SQLAlchemy/PostgreSQL
        dtype_mapping = {
            'object': types.VARCHAR(255),  # Strings gerais
            'category': types.VARCHAR(255), # Categorias como VARCHAR
            'datetime64[ns]': types.Date,  # Datas (apenas data, sem tempo)
            'int64': types.BigInteger,     # Inteiros grandes
            'int32': types.Integer,        # Inteiros normais
            'float64': types.Float(precision=53), # Floats
            'float32': types.Float(precision=24), # Floats menores
            'bool': types.Boolean          # Booleanos
        }
        
        # Mapeia os dtypes do DataFrame para os dtypes do SQLAlchemy
        # Verifica se o dtype está no mapping, caso contrário, deixa o Pandas inferir ou define um default
        table_dypes = {col: dtype_mapping.get(str(df_gold[col].dtype), None) for col in df_gold.columns}
        
        # Para colunas de taxa (float que representam percentuais), pode ser DECIMAL para maior precisão
        # Ajuste a precisão e escala conforme a necessidade. DECIMAL(5,4) significa 5 dígitos no total, 4 após a vírgula.
        if 'taxa_inadimplencia_final_segmento' in df_gold.columns:
            table_dypes['taxa_inadimplencia_final_segmento'] = types.DECIMAL(precision=10, scale=8) # Maior precisão
        if 'perc_ativo_problematico_final_segmento' in df_gold.columns:
            table_dypes['perc_ativo_problematico_final_segmento'] = types.DECIMAL(precision=10, scale=8) # Maior precisão
        ...
        df_gold.to_sql(
            ...
            index=False,
            chunksize=5000, # Ajuste se necessário para performance/memória
            dtype=table_dypes # Usa o mapeamento de tipos
        )
...
if __name__ == '__main__':
    ...
    # Baseado na sua estrutura de arquivos: de 2024-01 a 2025-05
    start_date_process = date(2024, 1, 1) # Primeiro mês de dados
    end_date_process = date(2025, 5, 1)   # Último mês de dados
    ...
    # Isso garante que a tabela seja criada corretamente e os dados sejam adicionados incrementalmente.

    # Lista todos os arquivos Parquet na pasta Gold/scr-agregado para o período definido
    files_to_load = []
    ...
    while current_date_loop <= end_date_process:
        ...
        gold_file_path_aggr = os.path.join(GOLD_AGGR_INPUT_PATH, str(year), gold_file_name_aggr)
    ...
    if not files_to_load:
        ...
        first_file_path = sorted(files_to_load)[0] # Garante que o primeiro arquivo é o mais antigo
        ...
        try:
            ...
            remaining_files_to_load = [f for f in files_to_load if f != first_file_path]
